...
import com.pangubpm.bpm.model.xml.impl.type.reference.ReferenceImpl;
...
public class ModelElementInstanceImpl implements ModelElementInstance {
  ...
  public Collection<ModelElementInstance> getChildElementsByType(ModelElementType childElementType) {
    ...
    List<ModelElementInstance> instancesResult = new ArrayList<ModelElementInstance>();
    int count=0;
    for (int i = 0; i < instances.size(); i++) {
      //TODO 解析可能会内存泄露，后续版本解决
      ModelElementInstance modelElementInstance = instances.get(i);

      if (count<=4){
        instancesResult.add(modelElementInstance);
      }
      count++;
    }
